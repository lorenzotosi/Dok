...
\begin{itemize}
    \item \textbf{Gestione Cartelle:} Creare, eliminare e organizzare cartelle in una struttura gerarchica (cartelle nidificate).
    \item \textbf{Gestione Documenti:} Creare nuovi documenti di testo all'interno di una determinata cartella o nella radice del proprio spazio di lavoro. Può rinominare ed eliminare i propri documenti.
    \item \textbf{Visibilità Risorse:} Configurare la visibilità delle proprie risorse (cartelle e documenti) come "privata" (accessibile solo al proprietario ed a specifici invitati) o "pubblica" (accessibile a chiunque tramite link, compresi gli ospiti).
    \item \textbf{Condivisione:} Condividere documenti privati con altri utenti registrati inserendo la loro email e assegnando un ruolo specifico:
    \begin{itemize}
    ...
    \item \textbf{Sistema di Notifiche:} Ricevere notifiche in tempo reale all'interno dell'applicazione quando un documento viene condiviso con lui, quando vengono modificati i suoi permessi o quando un documento condiviso viene rimosso dal proprietario.
\end{itemize}

\subsubsection{Amministratore (Admin)}
L'utente con privilegi amministrativi può:
\begin{itemize}
    \item \textbf{Elenco Utenti:} Accedere ad un pannello di controllo contenente la lista di tutti gli utenti iscritti alla piattaforma.
    \item \textbf{Presenza Real-Time:} Monitorare lo stato di presenza degli utenti in tempo reale (online o offline) e visualizzare la data dell'ultimo accesso (\textit{lastSeen}) per gli utenti non connessi.
    \item \textbf{Dettagli e Metriche Utente:} Esaminare il profilo specifico di un utente per visualizzare metriche quali il numero di documenti creati, condivisi e ricevuti.
    \item \textbf{Ispezione File System:} Sfogliare l'albero delle cartelle e dei documenti di un qualsiasi utente registrato per scopi di moderazione e supporto.
    \item \textbf{Gestione Ruoli:} Modificare il ruolo di un utente promuovendolo ad amministratore o revocandone i privilegi. Il sistema notifica immediatamente l'utente interessato in caso di variazione del ruolo.
\end{itemize}
