...
\author{
  ...
  \and
  Alessandro Stefani \\ \emailaddr{alessandro.stefani10@studio.unibo.it}
}
...
\begin{document}
  ...
  \begin{abstract}
    Dok is a distributed, real-time collaborative document editing platform implemented as a web application.
    It allows multiple users to create, share, and simultaneously edit text documents.
    The system is built on a client--server architecture in which the backend is horizontally scaled across
    two \textbf{Node.js/Express} server instances, load-balanced by an Active-Passive \textbf{Nginx} cluster managed
    via \textbf{Keepalived} (VRRP).
    Real-time collaboration is achieved through
    ...
    Pub/Sub adapter, which also enables cluster-wide user-presence tracking.
    Persistent state is stored in a \textbf{MongoDB} document database.
    Documents are organised in folders and can be made public or private, with document-level
    sharing policies (\textit{viewer}, \textit{commenter},\textit{editor}) enforced at the service layer.
    Global platform access and administrative privileges are secured through JWT-based authentication and Role-Based Access Control.
    An administrative dashboard provides real-time platform-wide statistics, audit trails, user presence information, and site-access logs.
    The backend infrastructure is containerised with \textbf{Docker Compose}, making deployment reproducible.
  \end{abstract}
  ...
  It combines a single-page application (SPA) frontend, built with Vue~3 and Vite, with a horizontally scalable REST and WebSocket backend.

  \subsection{Use-Case Description}
  Dok targets teams of knowledge workers, such as students, researchers, and professionals, who need
  to collaborate on written documents in real time from multiple locations.


  \paragraph{What the system does.}
  Users create rich-text documents (formatted paragraphs, headings, inline styling) and folders
  to organise them. They can create private documents, create them in a global feed, or share them
  ...
  comments allow asynchronous annotation. An integrated AI
  ...
  \begin{enumerate}
    ...
    The system shall save the Yjs document state to MongoDB within 3~seconds of the first
    unsaved update and when the last collaborator leaves.
  ...
  \begin{enumerate}
    ...
    \textbf{Node.js}, chosen for the ecosystem's WebSocket support.
    \item[\textbf{IC-2}] The frontend shall be a \textbf{Vue~3} SPA, chosen for its reactive
    component model and TypeScript integration.
    \item[\textbf{IC-3}] Real-time collaboration shall be built on \textbf{Yjs} CRDTs for
    operation-based collaborative editing without a central OT server.
  ...
  drops as permanent disconnections (network flap handling). The periodic checkpoint mechanism and
  ...
  setup minimizes operational costs.

  \subsection{Non-Relevant Distributed System Features}

  \paragraph{Strict consistency.}
  Strict consistency is not required for document editing. Yjs handles concurrent changes
  and allows each copy of the document to converge without a global lock.

  \paragraph{Heterogeneity.}
  Heterogeneity between backend instances is not relevant to the project. Both instances
  run the same Node.js application inside identical Docker containers.

  \paragraph{Economy and strict operating costs.}
  Strict operating costs were not a main requirement. Dok is an academic project designed
  to run through Docker Compose on standard hardware, without a specific cloud platform.
  ...
  \begin{itemize}
    \item \textbf{Nginx HA Cluster} (\texttt{nginx-master} and \texttt{nginx-backup}):
    Two instances of Nginx coupled with Keepalived, acting as an Active-Passive reverse proxy.
    Keepalived uses the VRRP protocol in \textit{unicast} mode to manage a floating Virtual IP,
    ensuring High Availability at the proxy layer.
    \item \textbf{Entrypoint} (\texttt{haproxy:alpine}):
    A lightweight TCP proxy used exclusively in the local Docker Compose environment
    to map the host's port 3000 to the internal Nginx VIP.
...
upstream websocket_cluster {
    ...
    server backend1:3000 max_fails=1 fail_timeout=3s;
    server backend2:3000 max_fails=1 fail_timeout=3s;
  ...
  Beyond the collaborative editing channel, Redis acts as the
  ...
  \begin{itemize}
    \item \textit{Document handler} (stateful, per document).
    On \texttt{join-document} it lazily loads the CRDT from MongoDB and registers a deep observer on the Yjs XML fragment.
    The observer inspects each CRDT transaction's \texttt{origin} (which the server sets to the editing user's id
    when applying a remote update) and accumulates inserted characters per user into \texttt{pendingUserChars};
    this provides attribution for the audit log without per-keystroke database writes. When the first
    unsaved update arrives, the handler schedules a checkpoint after 3~seconds. Further updates do
    not reset the timer, so the checkpoint stores the latest state collected during that interval.
    This keeps the database write load lower than the editing event rate.
    \item \textit{Presence manager} (stateful, per user).
    On connection it joins a per-user room \texttt{user:<id>}; because the room is shared across backends through Redis,
    the count of sockets in the room is a cluster-wide online indicator.
    To support multi-tab and multi-device sessions, it employs a dual-tracking strategy.
    Globally, it leverages the Redis adapter to count sockets across the cluster via \texttt{fetchSockets()}.
    Locally, it maintains a \texttt{Map<string, Set<string>>} in RAM mapping user IDs to their local socket IDs.
    On disconnect, it waits a 3-second \emph{grace period} before querying the global cluster state;
    if the cluster-wide socket count for that user reaches zero, it updates \texttt{lastSeen} in the database and
    emits \texttt{presence\_update} to the admin room. The local RAM map is crucial for the Graceful Shutdown flush.
  ...
  Document content is maintained as a Yjs \texttt{Y.Doc}. Yjs implements a CRDT based on
  the YATA algorithm. Its updates can be applied in any order and still converge to the same
  document state. This does not require a central coordinator or Operational Transformation.
  The server holds the in-memory state for each active document and periodically saves it to
  MongoDB.
  ...
  \paragraph{Load Balancer High Availability (Active-Passive).}
  To eliminate the reverse proxy as a SPOF, the infrastructure deploys two Nginx instances coupled with Keepalived.
  Using the Virtual Router Redundancy Protocol configured in \emph{unicast} mode
  (to cleanly bypass multicast routing restrictions of containerised environments) the nodes negotiate a floating Virtual IP.
  If the \texttt{nginx-master} container crashes, \texttt{nginx-backup} detects the missing heartbeats, claims the VIP,
  and assumes routing duties. Existing WebSocket connections are interrupted briefly while
  the clients reconnect through the backup instance.

  \paragraph{Development Environment vs. Production Topology.}
  It is worth specifying that the reference Docker Compose deployment introduces an \texttt{entrypoint} container (HAProxy)
  to easily route local host traffic (\texttt{localhost:3000}) to the internal Keepalived Virtual IP.
  While this container technically represents a Single Point of Failure within the local simulation, it is strictly a development artifact.
  In a production environment, this component would be absent, as external DNS records would direct client traffic
  straight to the VIP managed by the Active-Passive Nginx cluster.
  ...
  \paragraph{Periodic checkpointing and flush-on-close.}
  The save strategy combines a 3-second checkpoint with a final flush when
  \texttt{clientsCount} reaches zero. Under normal operation, this keeps the amount of
  unsaved editing data close to the checkpoint interval.
  ...
  \paragraph{Graceful Shutdown and Fast Failover.}
  To prevent data loss during container termination, the Node.js instances implement a synchronous state-lock mechanism.
  Upon receiving a \texttt{SIGTERM} or \texttt{SIGINT} signal, the backend raises an \texttt{isShuttingDown} flag that halts all asynchronous
  socket disconnection timers.
  It immediately performs a flush of the ephemeral RAM state (active Yjs CRDTs and local User Sessions) directly to MongoDB.
  Crucially, rather than waiting for long-lived WebSocket connections to drain naturally, which would cause
  the process to hang indefinitely, the server proactively severs all active sockets (\texttt{io.close()}).
  This forced TCP termination allows both the Nginx load balancer and the Vue.js clients to detect the node's
  unavailability. The clients then reconnect to the surviving replica using exponential backoff.
  An 8-second watchdog timer ensures the process exits cleanly even if database queries hang.

  \subsection{Availability}\label{availability}

  \textbf{Caching.} There is intentionally little caching: the in-memory \texttt{activeDocuments} map
  is effectively a \emph{session cache} of live documents, present only on the owning backend.
  The \texttt{tiptapJson} projection persisted alongside the CRDT acts as a pre-computed rendering cache
  so that read-only opens do not need to run the Yjs$\rightarrow$Tiptap transformation.

  \textbf{Load balancing.} Nginx performs load balancing with the consistent-hashing strategy described in \cref{infrastructure}.
  This simultaneously balances connection load and enforces session stickiness,
  which is what makes the single-replica-per-document design correct.
  For pure REST traffic (no \texttt{documentId}), client-IP hashing provides a fairer distribution.

  \textbf{Network partitioning.} The backend instances do not communicate directly, but both
  depend on Redis and MongoDB. If a backend loses its connection to Redis, cross-instance
  events and presence tracking stop working on that instance. If it loses MongoDB, persistence
  and database-backed API operations fail. The system does not provide formal partition
  tolerance and assumes a reliable internal network.
  ...
  \begin{enumerate}
    \item \textbf{Endpoint-level RBAC}: \texttt{requireAuth} validates the JWT signature,
    while \texttt{requireAdmin} implements a \textbf{Stale Data Defense} by querying the database
    to ensure the administrator privileges have not been revoked during the token's lifespan.
  ...
  \begin{description}
    ...
    \begin{itemize}
      ...
      returns \texttt{true}. After disconnection and the 3-second grace period, the user is
...
GEMINI_API_KEY=<your-google-api-key>
  ...
  The \texttt{docker-compose.yml} defines seven services:

  \begin{description}
    \item[\texttt{nginx-master}, \texttt{nginx-backup}]
    Built from a custom Dockerfile based on \texttt{nginx:alpine}, installing Keepalived.
    They are assigned static IPs on a custom subnet to allow VRRP unicast communication and VIP assignment,
    requiring \texttt{NET\_ADMIN} capabilities.
    \item[\texttt{entrypoint}] \texttt{haproxy:alpine} image; exposes port 3000 to the host and blindly forwards TCP traffic to the VIP (\texttt{172.20.0.100}).
...
#   http://localhost:3000/api-docs (Swagger UI)
  ...
  After approximately 30--60 seconds for image builds, all seven containers start. MongoDB and
...
# SPA available at http://localhost:5173
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[scale=0.3]{distribuiti_report/diagrammi/screen/login.png}
    \caption{Login.}
  \end{figure}
  \begin{figure}[H]
    \centering
    \includegraphics[scale=0.3]{distribuiti_report/diagrammi/screen/register.png}
    \caption{Register.}
  \end{figure}
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[scale=0.30]{distribuiti_report/diagrammi/screen/loggeduser.png}
    \caption{Dashboard.}
  \end{figure}
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[scale=0.3]{distribuiti_report/diagrammi/screen/doc.png}
    \caption{The document.}
  \end{figure}

  \begin{figure}[H]
    \centering
    \includegraphics[scale=0.3]{distribuiti_report/diagrammi/screen/share.png}
    \caption{Sharing.}
  \end{figure}
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[scale=0.3]{distribuiti_report/diagrammi/screen/notifica.png}
    \caption{Notifications.}
  \end{figure}

  \subsection{Administrator Dashboard}

  Administrators access the admin panel from the navigation bar.

  \begin{figure}[H]
    \centering
    \includegraphics[scale=0.3]{distribuiti_report/diagrammi/screen/admin/adminAccess.png}
    \caption{Admin Dashboard Access Point.}
  \end{figure}
  ...
  \begin{itemize}
    ...
    \begin{figure}[H]
      \centering
      \includegraphics[scale=0.3]{distribuiti_report/diagrammi/screen/admin/admin.png}
    ...
    \begin{figure}[H]
      \centering
      \includegraphics[scale=0.3]{distribuiti_report/diagrammi/screen/admin/user.png}
      \caption{User info.}
    \end{figure}

    \item \textbf{Dok detail}: Brief details about creation date, visibility and share of the selected Dok on the user's file-system tree (Dok and folders names are obscured if not public).

    \begin{figure}[H]
      \centering
      \includegraphics[scale=0.3]{distribuiti_report/diagrammi/screen/admin/dokDetails.png}
      \caption{Dok info.}
    \end{figure}
    ...
    \begin{figure}[H]
      \centering
      \includegraphics[scale=0.3]{distribuiti_report/diagrammi/screen/admin/dokLogs.png}
      \caption{Dok Access and Modification Logs.}
    \end{figure}

    \item \textbf{Statistics}: aggregated statistics across the platform (users, documents, folders,
    sharing activity, site-access trends by hour / weekday / day-of-month).
    \begin{figure}[H]
      \centering
      \includegraphics[scale=0.3]{distribuiti_report/diagrammi/screen/admin/stats.png}
    ...
    \begin{figure}[H]
      \centering
      \includegraphics[scale=0.3]{distribuiti_report/diagrammi/screen/admin/globalLogs.png}
      \caption{Access logs.}
    \end{figure}
  \end{itemize}
  ...
  \paragraph{Role.}
  Distributed Real-Time Collaboration, Nginx Routing, and WebSocket Connection Management.
  I designed and implemented the document-specific routing strategy for collaborative
  editing. Inspired by the Google Docs model, I configured Nginx to route all users
  editing the same document to the same backend instance. I also implemented the
  WebSocket connections, rooms, and events used for collaborative editing, user presence,
  and real-time dashboard updates.

  \paragraph{Strengths.}
  My primary strength was implementing the \textbf{Document-Specific Collaboration
  Connection} mechanism. Inspired by the Google Docs model, I designed each editor to
  create a dedicated Socket.IO connection containing the \texttt{documentId}. This allows
  the Nginx routing strategy to keep collaborators of the same document on a consistent
  backend instance. I also improved the join/leave lifecycle and presence handling,
  addressing stale or ``ghost'' socket connections.

  I also implemented real-time application features on top of the distributed
  infrastructure. I implemented the \textbf{Real-Time Dashboard Synchronisation} flow:
  when a public document is created, or when a document is shared with a user, the
  relevant dashboard updates immediately through Socket.IO events. Public-document events
  are delivered through the \texttt{global-dashboard} room, while sharing events are
  delivered through per-user rooms. These mechanisms leverage the existing Socket.IO
  Redis adapter to work across backend instances.

  Finally, I implemented integration tests for the distributed core. The tests start two
  real Socket.IO instances sharing Redis and MongoDB, and verify cross-instance presence,
  CRDT update propagation, and persistence of the final document state.

  \paragraph{Weaknesses.}
  I underestimated the complexity of propagating document-sharing and document-creation
  events in real time. Initially, I thought that emitting a Socket.IO event from the
  backend would be sufficient to update the user interface. Instead, I had to define the
  correct delivery scope for each event: public-document events are sent to the global
  dashboard room, while sharing events are sent to per-user rooms. The clients also need
  to subscribe to the relevant events and update their local state consistently.

  \subsection{Alessandro Stefani}

  \paragraph{Role.}
  Infrastructure, Security, and Distributed State Management.
  I focused on designing the stateless authentication architecture, cluster-wide observability,
  inter-node communication via the Redis backplane, and application-level fault-tolerance mechanisms.

  \paragraph{Strengths.}
  My primary strength was bridging the gap between application logic and distributed infrastructure.
  I designed the \textbf{Distributed Observability} system (the Admin Dashboard) inserting in our project the Redis Pub/Sub adapter
  to aggregate real-time WebSocket presence and audit logs across isolated backend instances.
  Furthermore, I engineered the \textbf{Graceful Shutdown} pipeline: overcoming the hanging callback problem
  of long-lived WebSockets, my implementation synchronously flushes ephemeral RAM data to MongoDB
  and then severs TCP connections to trigger an Nginx failover.
  Finally, I eliminated the load balancer SPOF by designing an \textbf{Active-Passive Nginx cluster}.
  By configuring Keepalived VRRP in \emph{unicast} mode, I bypassed the multicast traffic limitations of the Docker network,
  achieving High Availability at the proxy layer also.

  \paragraph{Weaknesses.}
  I underestimated the complexity of graceful teardowns in stateful WebSocket architectures,
  which led to race conditions between socket disconnection events and database closures
  (a problem I later fixed by introducing the synchronous state lock).
