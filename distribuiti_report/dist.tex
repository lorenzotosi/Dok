...
The entire infrastructure is containerised with \textbf{Docker Compose}, making deployment reproducible.
\end{abstract}
...
\begin{itemize}
  ...
  this provides attribution for the audit log without per-keystroke database writes. On every \texttt{crdt-update}
  it resets a 3-second debounce timer: when editing pauses for 3~seconds, the full binary state and the Tiptap JSON
  projection are persisted. This \emph{debounced persistence} is the main mechanism that keeps database write load
  proportional to \emph{editing sessions} rather than to \emph{keystrokes}, addressing the performance concern of \cref{ds-features}.
...
and assumes routing duties without dropping client traffic.
...
unavailability, triggering the exponential backoff and a fast failover to the surviving replica,
while guaranteeing data consistency.
